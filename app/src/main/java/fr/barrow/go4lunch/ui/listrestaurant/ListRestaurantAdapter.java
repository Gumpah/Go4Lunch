...
import fr.barrow.go4lunch.databinding.ListRestaurantItemBinding;
import fr.barrow.go4lunch.ui.ClickCallback;

public class ListRestaurantAdapter extends RecyclerView.Adapter<ListRestaurantAdapter.ListViewViewHolder> {
    ...
    public ListRestaurantAdapter(ArrayList<Restaurant> restaurants, ListRestaurantFragment listRestaurantFragment, ClickCallback callback, Location location) {
    ...
    @NonNull
    @Override
    public ListRestaurantAdapter.ListViewViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        mContext = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        return new ListViewViewHolder(ListRestaurantItemBinding.inflate(inflater, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull ListRestaurantAdapter.ListViewViewHolder holder, int position) {
    ...
    public static class ListViewViewHolder extends RecyclerView.ViewHolder {
        ...
        private ListRestaurantItemBinding binding;

        public ListViewViewHolder(ListRestaurantItemBinding b) {
            super(b.getRoot());
            binding = b;
        }
        ...
        void setClickListener(String restaurantId, ClickCallback callback) {
            binding.constraintLayoutRestaurantListItem.setOnClickListener(v -> callback.myClickCallback(restaurantId));
        }
        ...
        void setupClosingTime(Restaurant restaurant, Date now) {
            if (restaurant.getClosingTimeDate() == null) {
                ...
                if (diffInMinutesClosing <= 0 || diffInMinutesOpening < 0) {
                    ...
                    binding.textViewListRestaurantClosingTime.setTextColor(ResourcesCompat.getColor(binding.getRoot().getResources(), R.color.redClosingTime, null));
                } else if (restaurant.getClosingTime() != null){
